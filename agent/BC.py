...
def compute_and_log_NRC3(full_WWT, min_eigval, Sigma_sqrt, linspace_num=1000):
    WWT = full_WWT[-1, :].reshape(Sigma_sqrt.shape[0], -1)
    WWT_normalized = WWT / np.linalg.norm(WWT)

    c_to_plot = np.linspace(0, min_eigval, num=linspace_num)
    NRC3_to_plot = []
    for c in c_to_plot:
        c_sqrt = c ** 0.5
    ...
    # ===================================================
    full_WWT_normalized = full_WWT / np.linalg.norm(full_WWT, axis=1, keepdims=True)
    A_c = Sigma_sqrt - (best_c ** 0.5) * np.eye(Sigma_sqrt.shape[0])
    A_c = A_c / np.linalg.norm(A_c)
    all_NCR3 = np.linalg.norm(full_WWT_normalized - A_c.reshape(1, -1), axis=1) ** 2

    ep_to_plot = np.arange(full_WWT.shape[0])
...
def run_BC(config: TrainConfig):
    ...
    compute_and_log_NRC3(all_WWT,
                         train_theory_stats['min_eigval'],
                         Sigma_sqrt,
                         linspace_num=1000)
