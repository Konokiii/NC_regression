...
class TrainConfig:
    ...
    data_size: Union[int, float] = 1000  # Number of samples to use
...
# Save model
def save_model(save_name, epoch, model, config):
    suffix = f'_ep{epoch}.pt'
    save_folder = os.path.join(config.project_folder, f'models/{config.env}/{save_name}')
    os.makedirs(save_folder, exist_ok=True)
    save_path = os.path.join(save_folder, save_name + suffix)
    checkpoint = {
...
def compute_NRC1(features, target_dim, result_dict):
    ...
    for k in range(n_components):
        ...
        result_dict[f'NRC1nc_pca{k + 1}'] = torch.norm(
            H_proj_PCA_normalized_centered - H_normalized_centered).item() ** 2 / H.shape[0]
...
def wandb_init(config: dict) -> None:
    wandb.init(
        ...
        # Workaround for ERROR Run initialization has timed out after 60.0 sec.
    )
    # wandb.run.save()
...
class MujocoBuffer(Dataset):
    ...
    def _load_dataset(self, data_folder: str, env: str, split: str, data_size: int, normalize: str):
        if data_size <= 1:  # This means ratio/percentage.
            if split == 'train':
                file_name = f"{env}_top{data_size}.pkl"
            elif split == 'test':
                file_name = f"{env}_test.pkl"
            else:
                raise ValueError("Non-specified 'env/data_size/split'...")
            ...
            with open(file_path, 'rb') as file:
                ...
                if split == 'train':
                    ...
                    self.states = dataset['observations'][:20000]
                    self.actions = dataset['actions'][:20000]
                    self.size = 20000
            print('Successfully load dataset from: ', file_path)

            self.actions = np.arctanh(np.clip(self.actions, a_min=-1 + 1e-7, a_max=1 - 1e-7))

        else:  # This means the number of data points.
...
class Actor(nn.Module):
    ...
    def act(self, state: np.ndarray, device: str = "cpu") -> np.ndarray:
        ...
        # Modified: Apply tanh(), since the actor is predicting arctanh(a).
...
def run_BC(config: TrainConfig):
    ...
    save_model(save_name=config.name, epoch=0, model=actor, config=config)
    ...
    if config.lamH != -1:
        ...
        NRC3_target = (config.lamH / max(config.lamW, 1e-10)) ** 0.5 * (
                    Sigma_sqrt - (config.lamH * config.lamW) ** 0.5 * np.eye(Sigma_sqrt.shape[0]))
    ...
    for epoch in range(config.max_epochs):
        ...
        if (epoch + 1) in list(range(0, config.max_epochs + 1, config.max_epochs // 100)):
            # Save the initial model
            save_model(save_name=config.name, epoch=epoch + 1, model=actor, config=config)
